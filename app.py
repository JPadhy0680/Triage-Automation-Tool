...
# Version header
# v1.5.1-generalized-lot: Fix NameError (contains_company_product defined before use), correct newline join,
#                         generalized Lot No classification retained, Python 3.9-compatible type hints.
...
# Company product matcher (used early in parsing; must be defined BEFORE use)
...
# Generalized Lot No classification
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            for drug in root.findall('.//hl7:substanceAdministration', ns):
                ...
                if drug_id in suspect_ids:
                    ...
                    # Build product details ONLY for Celix-suspects (as per original behavior)
                    if matched_company_prod:
                        parts = []
                        ...
                        if lot_clean:
                            ...
                            if lot_kind == "text_other_company":
                                comments.append(
                                    f"Lot number '{lot_clean}' appears to be a company/manufacturer name (non-Celix) — please verify."
                                )
                            # exceptions and acceptable formats: no comment
                        ...
                        # Append parts using proper newline separator
                        if parts:
                            product_details_list.append(" \n ".join(parts))

                    # MAH mismatch comment (runs regardless of matched_company_prod)
...
with tab2:
    st.markdown("### \U0001F4CB Parsed Data Table \U0001F5C3️")
    if all_rows_display:
