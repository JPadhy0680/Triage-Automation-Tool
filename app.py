...
# Version header
# v1.5.0-generalized-lot: Generalized Lot No classification (numeric/alphanumeric OK; text-other-company flagged; exceptions ASKU/UNK/UNKNOWN)
...
- **Lot No rule (generalized)**: numeric/alphanumeric = OK; full text indicating other company → flagged; `ASKU/UNK/UNKNOWN` → no flag.
""") 
...
# Generalized Lot No classification (replaces competitor-name check)

def is_exception_token(value: str) -> bool:
    """Return True if the lot text is an allowed exception token (ASKU/UNK/UNKNOWN)."""
    if value is None:
        return False
    v = str(value).strip().lower()
    return v in UNKNOWN_TOKENS


def classify_lot_text(lot_text: str) -> str:
    """
    Classify Lot No text:
      - 'empty'              : empty string
      - 'exception'          : ASK/UNK/UNKNOWN tokens
      - 'numeric'            : only digits
      - 'alphanumeric'       : letters/digits + typical separators, NO spaces
      - 'text_celix'         : looks like company text and contains 'celix'
      - 'text_other_company' : looks like company text and does NOT contain 'celix'
      - 'mixed'              : everything else (contains spaces & digits/symbols)
    """
    if not lot_text:
        return "empty"
    lt = lot_text.strip()

    if is_exception_token(lt):
        return "exception"

    # Numeric only
    if re.fullmatch(r"[0-9]+", lt):
        return "numeric"

    # Alphanumeric (no spaces) with common separators
    if re.fullmatch(r"[A-Za-z0-9_\-/\.]+", lt):
        return "alphanumeric"

    # Text-like (letters + spaces/punct)
    if re.fullmatch(r"[A-Za-z][A-Za-z\s\.\,&\-()]{2,}", lt):
        if "celix" in lt.lower():
            return "text_celix"
        return "text_other_company"

    # Mixed content
    return "mixed"

MG_PATTERN = re.compile(r"(\d{1,3}(?:,\d{3})*\d+(?:\.\d{1,3})?)\s*mg", re.IGNORECASE) 
    ...
    if status == "launched_by_strength": 
        if isinstance(payload, dict) and payload: 
            if strength_mg is not None: 
                return payload.get(strength_mg if isinstance(strength_mg, (int, float)) else strength_mg) 
...
# (Removed DEFAULT_COMPETITOR_NAMES and contains_competitor_name)
                ...
                if drug_id in suspect_ids: 
                    ...
                    if matched_company_prod: 
                        ...
                        if lot_clean: 
                            ...
                            # Generalized lot classification
                            lot_kind = classify_lot_text(lot_clean)
                            if lot_kind == "text_other_company":
                                comments.append(f"Lot number '{lot_clean}' appears to be a company/manufacturer name (non-Celix) — please verify.")
                            elif lot_kind in ("empty", "exception"):
                                pass  # No warning
                            # numeric, alphanumeric, mixed => acceptable, no comment
                        ...
                        # MAH mismatch comment (unchanged)
...
# Note: contains_company_product retained from original code block
# (It was omitted earlier; re-adding here to avoid NameError)
